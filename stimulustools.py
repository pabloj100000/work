...
class stimulus:
    def __init__(self, stim, tsa=None, d=None):
        '''
        Generate a stimulus object consisting of the ndarray stim and an associated time stamp array (tsa) linking stimulus frames to time.

        stim:   your n dimension stimulus, last dimension is associated with time
        tsa:    time stamp array coming from the experimental computer
        d:      dictionary with experimental parameters, relies on having following keys:
                'endT':             end time of the experiment and assumes the experiment starts at t=0
                'framePeriod':      average flipping time of the monitor, currently with the monitor set at 10Hz is about 0.0100047
                'waitframes':       integer describing how over how many monitor flips the stimulus changes.
        '''
        self.stim = stim
        _pdb.set_trace()
        if tsa is not None:
            self.tsa = tsa
            self.deltaT = (tsa[1:]-tsa[:-1]).mean()
            self.endT = tsa[-1] + self.deltaT
        elif d:
            print(d)
            if 'framePeriod' not in d:
                raise ValueError('in generating stimulus object passed dictionary does not have "framePeriod" key')
            if 'waitframes' not in d:
                raise ValueError('in generating stimulus object passed dictionary does not have "waitframes" key')
            if 'endT' not in d:
                raise ValueError('in generating stimulus object passed dictionary does not have "endT" key')

            self.deltaT = d['framePeriod']*d['waitframes']
            self.endT = d['endT']
            self.tsa = np.arange(0, self.endT, self.deltaT)
        else:
            raise ValueError('either tsa or endT and deltaT are needed parameters')
    
    def upsamplestim(self, upfact):
        '''
        ...
        output
        ------
        stim object after upsampling time dimension by 'upfact'

        '''

        # Compute old and new sizes
        oldsz   = self.stim.shape
        newsz   = oldsz[:-1] + (upfact * oldsz[-1],)

        # Upsample the stimulus array
        stim_us = (self.stim.reshape((-1, 1)) * np.ones((1, upfact))).reshape(newsz)

        # Upsample the time stamp array
        x       = np.arange(0, upfact * self.tsa.size)
        xp      = np.arange(0, upfact * self.tsa.size, upfact)
        time_us = np.interp(x, xp, np.squeeze(self.tsa))

        stimObj_us = stimulus(stim_us, tsa=time_us)

...
def analysePD(wildcard, whiteThreshold=.8):
  ...
  #
  whiteFrameScans, scansperframe = loadAllWhiteFrames(wildcard, whiteThreshold)
  ...
  # for readability of following line, wfs is not a new array but a new name for whiteFrameScans
  wfs = whiteFrameScans
  
  # Keep only those white frames that signal the start of a new whiteFrameDistance
  # next line is somewhat complicated, to understand it right in a piece of paper the sequence of scans corresponding to whiteFrames
  ...
  # is why we have to compare 3 consecutive whiteFrameScans. And we always want to return the 1st element in the list
  wfs = [wfs[item] for item in range(len(wfs)-1) if wfs[item+1]+wfs[item-1]-2*wfs[item]>scansperframe/2 or item==0]
...
def loadAllWhiteFrames(wildcard, whiteThreshold):
  ...
  for file in os.listdir(os.getcwd()):
    if fnmatch.fnmatch(file, wildcard):
      ...
      accumulatedScans += meaRecording.getRecordingHeader(file)['nscans']
  
  return whiteFrameScans, scansperframe

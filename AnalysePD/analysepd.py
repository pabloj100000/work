...
s = '#{0:9}\t{1:10}\t{2:10}\t{3:10}\t{4:10}\t{5:10}\n'.format('binFile'.center(9),'startT'.center(10), 'endT'.center(10), 'length'.center(10), 'period'.center(10), 'nameOut'.center(10))
file_obj.write(s)
...
for prefix in prefixes:
	...
	startT, endT, length, period = photodiode.analysePD(prefix+'?.bin')

	# for each element in startT, endT, period, print them
	for i in range(len(startT)):

		s = '{0:10}\t{1:10.5f}\t{2:10.5f}\t{3:10.5f}\t{4:10.5f}\t\n'.format(prefix, startT[i], endT[i], length[i], period[i])
		file_obj.write(s)
